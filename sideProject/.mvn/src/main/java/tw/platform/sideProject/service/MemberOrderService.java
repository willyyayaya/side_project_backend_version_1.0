...
//-------------------------------    
    //站內信使用
    public void processAgreement(Order order, MemberOrder ownerOrder, boolean isAgreed) {
        // 找出該 order 下所有 wanted = true 的 MemberOrder
        List<MemberOrder> wantedMembers = memberOrderRepository.findByOrderAndWantedTrue(order);

        for (MemberOrder mo : wantedMembers) {
            if (isAgreed && ownerOrder.getOwned()) { 
                // 如果 owner 同意，且 owner 確實擁有該物品
                mo.setGetproject(null);
                memberOrderRepository.save(mo);

                // 轉換 Member 為 mimiMember
                mimiMember sender = new mimiMember(ownerOrder.getMember());  
                mimiMember receiver = new mimiMember(mo.getMember());

                // 發送內部訊息
                messageService.sendMessage(sender, receiver, 
                    "你申請的 " + order.getName() + " 已被批准", 
                    "你的申請的物品已獲得批准，請聯絡擁有者。");
            }
        }
    }
